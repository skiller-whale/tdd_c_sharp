...
public class TestBrowser(IPage page)
{
    private readonly IPage _page = page;

    public Task Visit(string url) => _page.GotoAsync(url);

    public async Task<bool> IsNewGameButtonVisible()
    {
        var newGameButton = await _page.QuerySelectorAsync("button.new-game");
        return newGameButton != null && await newGameButton.IsVisibleAsync();
    }
    
    public async Task ClickNewGameButton()
    {
        var newGameButton = await _page.QuerySelectorAsync("button.new-game");
        if (newGameButton == null)
        {
            throw new Exception("New game button not found in page");
        }
    ...
    {
        var statsElement = await _page.QuerySelectorAsync(".stats");
        if (statsElement == null)
        {
            throw new Exception("Stats element not found in page");
        }
        return (await statsElement.InnerTextAsync()).Trim();
    }
